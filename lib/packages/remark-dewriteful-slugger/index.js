...
function attacher() {
  ...
  function transformer(tree, file) {
    ...
    visit(tree, 'heading', function (node) {
      ...
      node.data.hProperties.class = cls;
    });
  }
  ...
  function parseAttribute(node) {
    ...
    if (startIndex < 0 || trimmedText.lastIndexOf(ATTR_END) !== trimmedText.length - 1){
      return null;
    }
...
function factory (original) {
  ...
  function atxHeadingTokenizer(eat, value, silent) {
    ...
    if ( !this.options.headerAttribute
      || silent
      || !result
